...
function TextPost(props) {
  const { id, title, date, body, caption, comments, updatePage, updateComments, toggleModal, userState } = props;
  const [jenBlogName, setJenBlogName] = useState(localStorage.getItem("jenBlogName"));
  const [commentState, setCommentState] = useState([]);
  const [toggleCommentState, setToggleCommentState] = useState(true);
  ...
  const deleteTextPost = () => {
    API.deleteTextPost(id).then(() => {
      API.deleteTpComments(comments._id).then(() => {
        updatePage();
        toggleModal(id + "textpost-modal");
      }).catch((errr) => { console.log(errr) })
    }).catch((err) => { console.log(err) });
  }

  const toggleComments = () => {
    let commentsOpen = toggleCommentState;
    if (commentsOpen) {
    ...
    setToggleCommentState(commentsOpen);
  }
      ...
      {userState ?
        <div>
          <div className="absolute top-0 right-0 w-8 h-8 text-lg text-center text-white font-bold opacity-50 bg-black rounded-full cursor-pointer"
            onClick={() => toggleModal(id + "textpost-modal")}
          >X
          </div>

          {/* modal background here */}
          <div className="modal-bg hidden opacity-50 z-10 bg-black fixed top-0 left-0" id={id + "textpost-modal-bg"}></div>

          {/* modal here */}
          <div id={id + "textpost-modal"}
