...
function TextThumbnail(props) {
  ...
  const { id, body, date, related, title, comments, tags } = props;

  useEffect(() => {
    // check if text is too tall. If so, show fade out effect if not related post
    if (!related) {
  ...
  return (
    <Link to={"/text/" + id}>
      <div className="thumbnail-card mb-4 cursor-pointer p-4" id={"card-" + id}>
